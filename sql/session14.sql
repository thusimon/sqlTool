...
-- COUNT(*) will return some rows when LEFT JOIN even if there is no rating
SELECT
  first_name,
  last_name,
  ...
  CASE
    ...
    ELSE 'Inactive'
  END AS Status,
  IF (COUNT(rating) > 0, 'Active2', 'Inactive2') AS 'Status(if)'
FROM reviewers
LEFT JOIN reviews ON reviews.reviewer_id = reviewers.id
GROUP BY reviewers.id;

SELECT title, rating, CONCAT_WS(' ', first_name, last_name) FROM series
JOIN reviews ON reviews.series_id = series.id
